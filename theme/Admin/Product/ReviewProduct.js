...
let productId = 172; // Default or replace with dynamic value

// Fetch reviews for the given productId
async function fetchReviews(productId) {
  const url = `https://localhost:44332/GetAllReview`; // Ensure endpoint supports filtering by productId
  try {
...
async function deleteReview(reviewId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://localhost:44332/api/Products/${reviewId}`, {
          method: 'DELETE',
          headers: {
...
async function approveReview(reviewId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://localhost:44332/Admin/${reviewId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
