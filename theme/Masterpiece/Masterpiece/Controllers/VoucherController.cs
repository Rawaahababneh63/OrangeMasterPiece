...
namespace Masterpiece.Controllers
{
    ...
    public class VoucherController : ControllerBase
    {
        ...
        {
            var existingVoucher = _db.Vouchers.FirstOrDefault(v => v.Code == voucher.Code);
            if (existingVoucher != null)
            {
                return BadRequest("Voucher code already exists.");
            }

            if (string.IsNullOrEmpty(voucher.Code) || voucher.DiscountAmount <= 0 || voucher.ExpiryDate <= DateTime.Now)
            {
            ...
            _db.Add(vouchers);
            _db.SaveChanges();
            SendVoucherToUser(userEmail, voucher.Code);
