    ...
    paginatedItems.forEach(product => {
        productContainer.innerHTML += `
            ...
            <td>${product.productId}</td>
                ...
                <td>${product.price}</td>
                <td><img src="${product.image}" alt="${product.name}" class="img-fluid" style="height: 50px;"></td>
                 <td>${product.stockQuantity}</td>
                  <td>${product.subcategoryId}</td>
                <td>${product.priceWithDiscount}</td>
                
                <td><button class="btn btn-warning" onclick="getProductId(${product.productId})">Edit</button>
